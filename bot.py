...
class Bot(commands.Bot):
    def __init__(self, browser):
        self.data = json.load("settings.json")
		super().__init__(irc_token=self.data["oauth"], client_id=self.data["client_id"], nick="AIDungeonBot", prefix='!',
                         initial_channels=[self.data["channel"])
        self.prompts = []
        self.authors = []
        self.voters = []
    ...
    async def clear_list(self):
        while True:
            ...
            if self.prompts:
                ...
                self.prompts.clear()
                self.authors.clear()
                await self.channel.send(f"Prompted! You have {self.cooldown} seconds to write the next prompt!")
    ...
    async def get_prompt(self, ctx):
        if ctx.author.name not in self.authors:
            self.prompts.append(ctx.message.content)
    ...
    async def start_voting(self, ctx):
        self.is_voting = True
        await self.channel.send("Starting a vote for reset! Say !y to vote for reset, and !n to vote against! Voting goes for 2 minutes!")
        await asyncio.sleep(120)
        if self.votes['y'] > self.votes['n']:
            self.browser.reset_game()
            await self.channel.send("Restarting the game...")
        else:
            await self.channel.send("The vote failed. Next voting is available in 5 mins.")
    ...
    async def vote_yes(self, ctx):
        if ctx.author.name not in self.voters and self.is_voting:
			...
			await self.channel.send("Vote accepted!")
        else:
            pass
